...
object BPSPythonPipeJobContainer {
    def apply(componentProperty: Component2.BPComponentConfig): BPSPythonPipeJobContainer =
        new BPSPythonPipeJobContainer(componentProperty)
}
 ...
 * }}}
 */
@Component(name = "BPSPythonPipeJobContainer", `type` = "BPSPythonPipeJobContainer")
class BPSPythonPipeJobContainer(override val componentProperty: Component2.BPComponentConfig) extends BPSJobContainer {
...
//        val jobMsg = event.date
        ...
        val metadataPath: String = jobMsg.getOrElse("metadataPath", "alfred-test-data/metadata").toString
        val filesPath: String = jobMsg.getOrElse("filesPath", "alfred-test-data/contents").toString
