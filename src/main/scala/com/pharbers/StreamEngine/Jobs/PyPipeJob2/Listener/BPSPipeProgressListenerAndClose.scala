 ...
 */
case class BPSPipeProgressListenerAndClose(override val job: BPSPythonPipeJob,
                                           spark: SparkSession,
                                           rowLength: Long,
...
//        val rows = hdfsfile.readHDFS(rowRecordPath).map(_.toLong).sum
        val rows = hdfsfile.readHDFS("/user/alfredyang/test/rowcheck/" + job.id).map(_.toLong).sum
