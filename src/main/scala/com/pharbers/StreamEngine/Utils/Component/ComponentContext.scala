...
trait ComponentContext{
    ...
    def buildComponent[T](config: ComponentConfig): T

    def buildComponent[T](id: String, name: String, args: List[AnyRef], config: Map[String, String]): T

    def getComponent[T](id: String): T

    def createComponent[T](id: String, needAppend: Boolean = false): T
}

case class ComponentConfig(id: String, name: String, args: List[String], config: Map[String, String])

class Component(val name: String, val `type`: String, val factory: Option[String] = None) extends annotation.StaticAnnotation
